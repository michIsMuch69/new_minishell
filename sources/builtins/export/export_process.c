...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/07/18 11:43:35 by jedusser          #+#    #+#             */
/*   Updated: 2024/07/18 13:14:18 by jedusser         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "exec.h"

int	var_for_export(t_vars *vars, char *env_var, char **export, int index)
{
	vars->equal_pos = ft_strchr(env_var, '=');
	if (vars->equal_pos)
	{
		vars->key = ft_substr(env_var, 0, \
		ft_strlen(env_var) - ft_strlen(vars->equal_pos));
		if (!vars->key)
			return (1);
...
int	init_exported_env(t_data *data, t_table *export)
{
	...
	{
		if (var_for_export(&vars, data->env.tab[i], export->tab, i) != 0)
