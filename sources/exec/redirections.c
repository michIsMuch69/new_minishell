...
/*   Updated: 2024/07/18 12:29:09 by jedusser         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	redir_output(t_data *data)
{
	...
	output_file = skip_redir_symbol(data->output.tab[(data->output.size - 1)],
			1);
	if (!output_file)
		return (-1); // crash
	if (arrow_count(data->output.tab[(data->output.size - 1)], '>') == 1)
		...
		output_fd = open(output_file, O_WRONLY | O_CREAT | O_APPEND | O_CLOEXEC,
				0644);
	else
